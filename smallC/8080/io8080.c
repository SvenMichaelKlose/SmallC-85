...
int unit;
{
    ...
    if (modes[unit = unit - UNIT_OFFSET]
        == READ) {
        if (bptr[unit] >= eptr[unit]) {
            ...
            eptr[unit] = (bptr[unit] = buffaddr (unit))
                + (i * BLKSIZ);
        }
        c = (*(bptr[unit]++)) & 0xff;
        if (c == CPMEOF) {
...
int unit;
{
    ...
    if ((unit != stdin) | (unit !=
        ...
        if (bptr
            ...
            while (bptr[unit] > (buffa = buffa + BLKSIZ));
            bdos (SETDMA, DEFAULT_DMA);
        }
    }
    return NULL;
...
char *name, *mode;
{
    ...
    if (EOF != (fileno = allocunitno ())) {
        /* internal file # excludes units 0,1 & 2
           ...
           these units */
        movname (clearfcb
                 (fcbaddr (fno2 = fileno - UNIT_OFFSET)),
                 name);
        if ('r' == *mode) {
            if (bdos1 (CPMOPEN, fcbaddr (fno2)) != CPMERR) {
                modes[fno2] = READ;
                // ptr>bufsiz => buffer empty 
              eptr[fno2] = bptr[fno2] = buffaddr (fno2 + 1 < tel:+1 >);
                return fileno;
            }
        } else if ('w' == *mode) {
