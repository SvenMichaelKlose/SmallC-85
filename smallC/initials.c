                   ...
                   int type, int value, TAG_SYMBOL * tag)
{
    ...
    if (find_symbol_initials (symbol_name) == 0) {
        add_symbol_initials
            (symbol_name, tag == 0 ? type : STRUCT);
    }
    if (tag != 0) {
        // find number of members, dim is total number of values added 
        int index =
            initials_table
            [initials_idx].dim % tag->number_of_members;
        int member_type =
            member_table[tag->member_idx + index].type;
        // add it recursively 
        add_data_initials (symbol_name,
                           member_type, value, 0);
    } else {
        ...
        if (type & CCHAR) {
            ...
            initials_data_table
                [initials_data_idx +
                 position] = (0xff00 & value) >> 8;

            initials_data_table
                [initials_data_idx +
                 position + 1] = 0xff & value;
            initials_table
                [initials_idx].data_len += INTSIZE;
        }
             ...
             int position, TAG_SYMBOL * tag)
{
    ...
    if (find_symbol_initials (symbol_name) != 0) {
        if (initials_table[initials_idx].type & CCHAR) {
            result =
                initials_data_table
                [initials_data_idx + position];
        } else if (initials_table[initials_idx].type & CINT) {
            position *= INTSIZE;
            result =
                (initials_data_table
                 [initials_data_idx +
                  position] << 8) + (unsigned char)
                initials_data_table
                [initials_data_idx + position + 1];
        } else
            if (initials_table
                [initials_idx].type == STRUCT) {
            ...
            for (i = 0;
                 i < (position % number_of_members); i++) {
                type =
            ...
            if (type & CCHAR) {
                result =
                    initials_data_table
                    [initials_data_idx + index];
            } else {
                result =
                    (initials_data_table
                     [initials_data_idx
                      + index] << 8) + (unsigned char)
                    initials_data_table
